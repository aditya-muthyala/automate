...
class HPNA:
  ...
  def extract_ip(self, portAddress_str):
	portAddress_str_split = portAddress_str.split(',')
	address_str = portAddress_str_split[0]
	mask_str = portAddress_str_split[1]
	address_match_set = re.search('\d+.\d+.\d+.\d+', address_str)
	mask_match_set = re.search('\d+.\d+.\d+.\d+', mask_str)
	portAddress = None
	portMask = None
	if address_match_set is not None:
		portAddress = address_match_set.group(0)
	if mask_match_set is not None:
		portMask = mask_match_set.group(0)
		
	return portAddress, portMask
  ...
  def get_vserver_ip(self, devip):
    ...
    try:
      ...
      for port in ports:
        ...
        '''
        if portType.find('VIP|VLAN') != -1 and portAddress_str:
          '''
          extract_ip() is used to parse the ipAddress field
          see above for the format
          '''
          portAddress, portMask = self.extract_ip(portAddress_str)
          portType.find('VIP|VLAN')
